...
/* Handles calculating and rendering of a single or multi-series cumulative line plot.
*/
var CumulativeSeries = React.createClass({
  ...
  render: function() {
    ...
    data.map(function(group,i) {
      yMax[i] = d3.sum(group, function(value) {return Math.abs(value[props.yMeasurement]);});
      yMin[i] = -yMax[i];
    ...
    sortedData.map(function(group,j) {
      ...
      circles[j] = group.map(function(value,i) {
        ...
        // Create an accumulator for the y-axis
        var x = xScale(dateParse(value[props.xMeasurement]));
        accumulator += Number(value[props.yMeasurement]);
        var y = yScale(accumulator);
    ...
    // Draws a line for each group through that group's points using the Line component
    var lines = sortedData.map(function(group,j) {
      ...
      return(
        <Line ref={"line" + j} path={path} color={props.colors(j)} key={j}/>
      )
    })
...
module.exports = CumulativeSeries;
